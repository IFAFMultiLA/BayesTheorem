...
So that we can calculate well later, we first define two events and their negation. These correspond to the possibilities mentioned above:

- $K$: Person sick
- $\overline K$: Person healthy
- $T$: Test positive
...
- $K$: Person sick and $\overline K$: Person healthy
...
$P(T \mid K)$ is called *Sensitivity* and is a quality measure for a test, as we have already seen in the package insert. Sensitivity indicates how high the probability is that the test shows a positive result for a person who is actually sick and is therefore also called *True Positive Rate*.

To calculate the actually sought *positive predictive value*, i.e., the conditional probability $P(K \mid T)$, we can use  **Bayes' Theorem**:

$$
P(K \mid T) = \frac{P(T \mid K) P(K)}{P(T)}.
$$

So what we need to calculate $P(K \mid T)$ are three probabilities: $P(K)$, $P(T \mid K)$, and $P(T)$.

::: summary

#### Definition of Conditional Probabilities

$P(K \mid T)$ – Probability that a person is actually sick, given that the test is positive
...
$P(K)$ indicates how high the probability is generally to be infected with Corona at this time – completely independent of a test result. This value is also called *Prevalence.* We can never know this probability exactly, but we can take an estimated value that corresponds to the proportion of currently infected people in Germany. Let's first set $P(K) = `r prevalence * 100`\%$. This is roughly the prevalence during the first wave of Corona in spring 2020 in Germany.

One might also come up with the idea of reading $P(K)$ from the study in the package insert, but this is not sensible in most cases: the study that led to the results in the package insert precisely examines how well the test works in sick people, so that people with proven Covid infection were specifically tested, so that this cannot be spoken of as a cross-section of the population.

::: summary

#### Definition of Conditional Probabilities

$P(T \mid K)$ – Probability that a test shows a positive result, given that the person is actually sick (Sensitivity)

:::

The probability $P(T \mid K)$ is already given in the package insert as Sensitivity.
...
![Tree diagram with both possibilities that lead to positive tests](images/baum3.png){height=300px}
...
However, this brings us two new problems: we need $P(T \mid \overline K)$ and $P(\overline K)$. Fortunately, we can calculate both via the probability of complementary events. Thus, $P(\overline K)$ (the general probability of *not* being sick) is easy to calculate, because we already know $P(K)$ (the general probability of being sick, estimated using prevalence): $P(\overline K) = 100\% - P(K) = `r (1 - prevalence)*100`\%$.

Finally, we are still missing $P(T \mid \overline K)$: the probability of having a positive test result when one is actually healthy (a *false positive result*). Here, another quality measure for tests helps us: the *Specificity*, also called *True Negative Rate*. It gives us the probability that a person receives a negative test result when they are actually healthy, so $P(\overline T | \overline K)$. The Specificity is often printed in the package insert of rapid tests, as we have already seen. As we can see in the tree diagram, $P(\overline T | \overline K)$ is complementary to $P(T | \overline K)$ and thus $P(T | \overline K) = 100\% - P(\overline T | \overline K)$.
...
$P(\overline T \mid \overline K)$ – Probability that a test shows a negative result, given that the person is actually healthy (Specificity)

:::

### Calculation

To calculate, for example, how high the probability is that a person is actually sick if they receive a positive test result from our considered rapid test, we note the Sensitivity and Specificity from the package insert again:

- Sensitivity: $P(T \mid K) = `r sens * 100`\%$
- Specificity: $P(\overline T \mid \overline K) = `r spec * 100`\%$

For the Prevalence we had assumed $P(K) = `r prevalence * 100`\%$.
...
```{r P_K}
question_text(
  "Let's assume that the study that evaluated the rapid test reflects the actual disease rate in the population (i.e., the prevalence). Calculate this prevalence $P(K)$ from the data in the confusion matrix, shown in the summary on the right side.

Enter either a mathematical expression like `(20+11)/10` or a number like `3.1`
(Caution, the decimal point is the dot).",
  answer_fn(function(input) {
...
```{r P_T}
question_text(
  "Calculate the total probability $P(T)$ that a test is positive, regardless of the health status of the tested person. This time, assume a prevalence of $P(K)=0.25$, but use the values of Sensitivity and Specificity from the package insert, i.e., $P(T \\mid K) = 0.914$ and
...
```{r P_K_T}
question_text(
  "Calculate the probability $P(K|T)$ that a person who tested positive is actually sick.
This time, assume a prevalence of $P(K)=0.25$ as well as a total probability of a positive test
of $P(T)=0.233$, but use the values of Sensitivity and Specificity from the package insert, i.e.
$P(T \\mid K) = 0.914$ and $P(\\overline T \\mid \\overline K) = 0.994$. Compare the result with the
result generated in the example calculation with a prevalence of $P(K) = 0.2\\%$.
...
```{r P_nK_nT_berechnet}
question_text(
  "You want to calculate the probability that a person who has a negative rapid test is actually healthy. Assume a prevalence of $P(K)=0.2\\%$. For the quality of the rapid test, use the values of Sensitivity and Specificity from the package insert, i.e. $P(T \\mid K) = 0.914$ and
$P(\\overline T \\mid \\overline K) = 0.994$.

Hint: Try applying Bayes' theorem for the conditional probability $P(\\overline K \\mid \\overline T)$.
...
In this learning application, you will also be given tasks to solve using R code. These tasks involve short code snippets that you should either complete (by replacing placeholders with corresponding code) or write entirely on your own. In most cases, you will generate a final result, **which must be in the last line.**
...
As you can see, you need to write the final result in the last line of the code input.
...
In the following code, the confusion matrix `confmat` is generated with the values from the package insert. Calculate $P(T \mid K) = \frac{P(T, K)}{P(K)}$ directly using `confmat`. You can access the value in row `i` and column `j` by using the syntax `confmat[i, j]`. You can select the entire row `i` using `confmat[i,]` and the entire column `j` using `confmat[,j]`.
...
Select the corresponding value for $T \cap K$ (positive test *and* sick) from `confmat` using `confmat[i,j]`. Also, calculate the number of people who were proven to be sick as the sum of the "sick" column. You can do this by selecting the column and using the `sum()` function. Then, divide the count for $T \cap K$ by the count for $K$.
</div>

## The Influence of Prevalence and Test Accuracy

Let's take another look at the calculation of the conditional probability $P(K \mid T)$:
...
This equation clearly shows the significant influence of prevalence on the overall result: Even with a high sensitivity of the test, the numerator $P(T \mid K) P(K)$ and thus the overall result become very small when the prevalence is very low. This also makes intuitive sense: If there are very few cases of a disease overall, the probability that a person is specifically affected by it, even if the test is positive, is comparatively low. In contrast, the probability is very high that a person is affected by a disease with many cases (i.e., high prevalence) regardless of how well the test performs.

We can also visualize this by plotting $P(K \mid T)$ as a function of prevalence for fixed sensitivity and specificity:
...
ggplot(ppv_fixed_sens_spec, aes(prev, ppv)) +
    ...
    labs(title = expression("Positive Predictive Value P" * (K ~ "|" ~ T ) ~ "as a Function of Prevalence"),
         subtitle = sprintf("With Sensitivity of %.1f%% and Specificity of %.1f%%.", fixed_sens * 100, fixed_spec * 100)) +
    theme_minimal()
```

We see how steeply the curve rises, and even with a prevalence of 10%, we can trust a test result with the given sensitivity and specificity. For lower prevalences, this is less justified, as shown here in more detail:
...
ggplot(ppv_by_param, aes(prev, ppv, color = spec_lbl, group = spec_lbl)) +
    ...
    labs(title = expression("Positive Predictive Value P" * (K ~ "|" ~ T )),
...
output$ppv_interactive <- renderPlot({
  ...
  ggplot(df, aes(prev, ppv)) +
    ...
    labs(title = expression("Positive Predictive Value P" * (K ~ "|" ~ T ) ~ "in Relation to Prevalence"),
...
question("Why does sensitivity not play such a strong role for the positive predictive value at low prevalence?",
  ...
  answer("Because at low prevalence, the term $P(T \\mid K) P(K)$ is always very small, so the sensitivity $P(T \\mid K)$ can hardly have an influence on the overall result.", correct = TRUE),
  allow_retry = TRUE
)
```

The specificity $P(\overline T \mid \overline K)$ has a great influence, because as we have already shown
...
and since $P(K)$ is very small, $P(\overline K)$ is very large and thus the specificity $P(\overline T \mid \overline K)$ has the "power" over the denominator through the term $(1 - P(\overline T \mid \overline K)) P(\overline K)$. Thus, a high specificity causes a small denominator and thus a high positive predictive value.

So we see: good tests (especially with high specificity) can make a big difference here especially at the beginning of an outbreak, because they allow infections to be detected quite reliably early on, even if the prevalence is still low.
...
Analogous to the graphs for the positive predictive value $P(K \mid T)$, we want to create a graph for the negative predictive value $P(\overline K \mid \overline T)$. This graph should show the negative predictive value as a function of prevalence and sensitivity.
...
Next, calculate the negative predictive value $P(\overline K \mid \overline T)$ for all given parameters and add the result as a column `nvw` to the Dataframe `npv_param`. Use a fixed specificity of $99.4\%$.
...
<ul>
    ...
    <li class="trackingdata-inputs">Changes to controls (for interactive graphics, etc.)</li>
...
**This procedure is solely for scientific research. It does not at any time draw conclusions about the respective person and thus has no effect on study achievements.**
